...
export function App() {
  ...
  async function runDockerCommand() {
    ...
    await ddClient.docker.cli.exec('run', [startCommand]);
  }
  ...
  function stopDockerCommand() {
    ddClient.docker.cli.exec('stop', [containerName]);
  ...
  function stopSpecificTunnel(tunnelName: string) {
    ddClient.docker.cli.exec('stop', [tunnelName]);
  }
  ...
  const [isSingleTunnelRunning, setIsSingleTunnelRunning] =
      ...
      {isTunnelVisible && (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <div>
              {Object.keys(
                JSON.parse(localStorage.getItem('tunnelToDataLogMap'))
              ).map(function (value) {
                return (
                  <div style={{ display: 'flex' }}>
                    <div
